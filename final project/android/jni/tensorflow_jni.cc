...
static std::string ClassifyImage(const RGBA* const bitmap_src,
  ...
  std::vector<tensorflow::Tensor> output_tensors(6);
  std::vector<std::string> output_names ({"output_length", "output_digit1", "output_digit2", "output_digit3", "output_digit4", "output_digit5"});
  ...
  std::vector<std::pair<float, int> > top_results;
  GetTopN(output->flat<float>(), 7, kThreshold, &top_results);

  std::vector<std::vector<std::pair<float, int>>> top_results_digit(5, std::vector<std::pair<float, int>>(1));

  for ( int i = 1; i < 6; i++ ) {
    VLOG(0) << "Reading from layer " << output_names[i];
    output = &output_tensors[i];
    GetTopN(output->flat<float>(), 1, kThreshold, &top_results_digit[i-1]);
  }

  float confidence = top_results[0].first;
  int length = top_results[0].second;
  std::stringstream seq_digits;

  float max_confidence = confidence;
  string max_seq_digits = "";
  ...
  for (const auto& result : top_results) {
    ...
    for ( int i = 0; i < length; i++) {
      confidence *= top_results_digit[i][0].first;
      seq_digits << top_results_digit[i][0].second;
    }
    if ( max_confidence > confidence ) {
      max_confidence = confidence;
      max_seq_digits = seq_digits.str();
      seq_digits.str("");
    }
  }
