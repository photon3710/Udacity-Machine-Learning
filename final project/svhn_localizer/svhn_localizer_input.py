  ...
  Returns:
    An object representing a single example, with the following fields:
      ...
      key: a scalar string Tensor describing the filename & record number
        for this example.
      box: a Tensor with the size(5,4).
...
def _generate_image_and_label_batch(image, box, min_queue_examples,
                                    batch_size):
  ...
  images, box_batch = tf.train.shuffle_batch(
      [image, box],
...
def distorted_inputs(data_dir, batch_size):
  ...
  return _generate_image_and_label_batch(float_image, read_input.box,
